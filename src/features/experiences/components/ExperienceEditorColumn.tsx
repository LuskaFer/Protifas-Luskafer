...
import type { CreateExperienceFormSchema } from '../zod/schemas'

interface ExperienceEditorColumnProps {
  isLoading: boolean
  thumbnailPreview: string | null
  register: UseFormRegister<CreateExperienceFormSchema>
  errors: FieldErrors<CreateExperienceFormSchema>
  onThumbnailSelect: (file: File) => void
  onRemoveThumbnail: () => void
}

export function ExperienceEditorColumn({
  isLoading,
  thumbnailPreview,
  register,
  errors,
    ...
    <div className="flex flex-col gap-6 overflow-y-auto p-6">
      ...
      <section className="space-y-3">
        <div className="grid grid-cols-[130px_1fr] items-center gap-4">
          ...
          <Input id="exp-title" {...register('title')} placeholder="Título da experiência" />
          {errors.title && (
        ...
        <div className="grid grid-cols-[130px_1fr] items-center gap-4">
          ...
          <Input id="exp-company" {...register('company')} placeholder="Nome da empresa" />
          {errors.company && (
            <p className="col-start-2 text-sm text-destructive">{errors.company.message}</p>
          )}
        </div>
        <div className="grid grid-cols-[130px_1fr] items-center gap-4">
          <Label htmlFor="exp-period">Período</Label>
          <Input id="exp-period" {...register('period')} placeholder="Ex: Jan 2023 — Dez 2024" />
          {errors.period && (
            <p className="col-start-2 text-sm text-destructive">{errors.period.message}</p>
          )}
        </div>
        <div className="grid grid-cols-[130px_1fr] items-start gap-4">
          <Label className="pt-2" htmlFor="exp-description">
            Descrição
          </Label>
          <textarea
            id="exp-description"
