...
interface ExperienceListColumnProps {
...
export function ExperienceListColumn({
  items,
  selectedId,
  isLoading,
  onSelect,
}: ExperienceListColumnProps) {
  return (
          ...
          return (
            ...
            >
              <div className="flex min-w-0 flex-1 flex-col gap-0.5">
                ...
                <span className="truncate text-xs opacity-70">{item.company}</span>
                <span className="truncate text-xs opacity-50">{item.period}</span>
                <div className="mt-1 flex items-center justify-between gap-2">
                  <span
                    className={cn(
                      'w-fit rounded-full border px-2 py-0.5 text-[10px] font-medium uppercase tracking-wide',
                      EXPERIENCE_STATUS_VARIANT_MAP[status] || '',
                    )}
                  >
                    {EXPERIENCE_STATUS_LABELS[status] || 'Desconhecido'}
                  </span>
                  {item.createdAt && (
