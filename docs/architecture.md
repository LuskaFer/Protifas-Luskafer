...
### Lei V: Componentes de UI "Estúpidos"
Os componentes de `src/shared/ui/` (botões, modais, inputs) não podem conter regras de negócio. O `button.tsx` não sabe se está logando um usuário ou salvando um PDF; ele apenas recebe `onClick`, `disabled`, `variant`, `size` e `children`.
